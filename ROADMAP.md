...
### תשתית וחוויית משתמש
- **גלריה סבב 3 — שלב א': סקריפט העברה ל-Blob (בלי מחיקה)**
  (`feature/gallery-blob-migration`, מקומי — ממתין ל-push; **הסקריפט טרם הורץ
  בפועל וה-SQL טרם נוצר**). [`scripts/migrate-gallery-to-blob.mjs`](scripts/migrate-gallery-to-blob.mjs)
