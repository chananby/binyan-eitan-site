...
### תשתית וחוויית משתמש
- **חילוץ מסרטון — תצוגה מוגדלת לפריימים** (`feature/frame-preview-enlarge`,
  מקומי — ממתין ל-push+deploy). רשת הפריימים הציגה ~10 תצוגות זעירות, שמהן אי
...
- **גלריה סבב 3 — שלב א': העברה ל-Blob מכפתור באדמין (בלי מחיקה)** — **נפרס**
  (20.7.26, `feature/gallery-blob-button` → main `3576fe5`, `dpl_4hrvUHzUCsq`;
  ה-endpoint מחזיר 401 admin-gated. **הכפתור טרם הורץ בפועל** — אף תמונה לא
  הועלתה, ה-DB לא נגע).
  ...
  - **שלב ב' בתור** (רק אחרי שחנן יאמת שהאתר מוגש מ-Blob): הכרעה לגבי שלושת
    ה-fallbacks שעדיין קוראים מ-`/public`, ורק אז מחיקה מ-`/public`.
