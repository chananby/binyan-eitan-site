...
- **פאנל בקשות תיקון — תצוגה נכונה לכל סוג + תרגום + התראת עריכה** — **נפרס**
  ...
  אימות `x-matched-path: /admin` + endpoint התרגום מחזיר 401 לא-מאומת). ⚠️ **SQL ההדגמה
  טרם הורץ — חנן מריץ ידנית** (`scripts/demo_correction_requests.sql`). הפאנל השתמש בתבנית אחת ("X ← Y" עם קו חוצה)
