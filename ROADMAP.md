...
### תשתית וחוויית משתמש
- **רוחב אחיד לכל לשוניות האדמין + ריסון אלמנטים מתוחים** — **מקומי, ממתין ל-push+deploy**
  (`fix/admin-uniform-width` @ `9e43722`). קודם רק דשבורד+board היו `max-w-7xl`, השאר
  ...
  - **בתור (לא תוקן — להחליט יחד):** טפסים שנמתחים ברוחב החדש — **AccountTab** (טופס
    סיסמה+הגדרות, בלי max-w), **Income/Expenses** (טופסי רישום), **ProjectsTab** (הוספת
    פרויקט), **PlanningTab** (טפסי אבן-דרך/משימה), **WorkersTab** (עריכת עובד, חלקי).
    לשוניות עם טבלאות/רשימות (Payroll, Attendance, Documents, Reports, Board, Quotes)
    **מרוויחות מהרוחב** — תקינות. build + 423 טסטים.
