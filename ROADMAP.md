...
### תשתית וחוויית משתמש
- **גלריית האדמין — תצוגה מקדימה בגדול בלחיצה** (`feature/gallery-admin-preview`,
  מקומי — ממתין ל-push+deploy). התצוגות המקדימות ברשת קטנות מכדי לבחור תמונת שער
...
- **גלריה — תג "דף פרויקט" מוסתר כשאין דף** — **נפרס** (20.7.26,
  ...
  **מונע חזרה של הבאג** בכל פרויקט עתידי שייווצר באדמין.
  - **soft-404 — נחקר, טרם תוקן.** `/[he|en]/projects/<slug-לא-קיים>` מחזיר
    **HTTP 200** עם דף שגיאה (ניתן לאינדוקס בגוגל). **שורש שאותר:**
    [`layout.tsx:163`](src/app/layout.tsx#L163) קורא `await headers()` → ה-root
    layout דינמי → שום דף לא מרונדר סטטית, וה-route נמצא במצב היברידי שבו סטטוס
    ה-404 של `notFound()` אובד. **שתי גישות נוסו ונכשלו באימות מקומי**
    (`dynamicParams=false` — חסר תוקף כשאין SSG; `dynamic="force-dynamic"` — לא
    שינה סטטוס), ולכן **הוחזרו** ולא נשלחו. מעניין: `expertise/[slug]` כן מחזיר
    404 תקין — ההבדל טרם בודד. **בתור:** לחקור ולתקן בנפרד.
