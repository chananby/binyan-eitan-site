...
- **תפריט צד לפורטל האדמין — שלב 2 (בנייה מאחורי דגל)** — **נפרס**
  ...
  - **שלב 3 — בתור:** ליטוש + אינטגרציית layout (הזזת אזור התוכן במקום padding) + החלפת
    ברירת המחדל והסרת השורה, אחרי אישור חנן.
...
- **מחולל ההצעות — ליטוש הצעת-היקף (בלי מחירים)** — **מוכן, לא נפרס**
  (`fix/quote-scope-only-polish` → `5ef2aa1`; build + 423 tests עוברים; ממתין
  לאישור פריסה). נובע מהצעה אמיתית (#499, משפחת חזן) שמפרטת היקף בלי מחיר לפריט.
  תצוגה/עיצוב בלבד — `effectiveBase`/`subtotalAfterDiscount`/`totalOverride`/לוח
  התשלומים לא נגעו.
  - **סה"כ פרק ריק** — שורת "סה"כ &lt;שם&gt;" הדפיסה ₪0.00 כשסכום הפרק 0. נוסף
    `total > 0` לתנאי (שמירת ה-guard `section.name` מסבב ב' לפרקים בלי שם — מקרה נפרד).
  - **הערת מע"מ יתומה** — "כל הסכומים בטבלה זו הם לפני מע"מ" (סבב ב') הופיעה גם
    בלי פריט מתומחר. עכשיו מותנית ב-`grandTotal() > 0`.
  - **שם קובץ PDF** — `updateDocumentTitle()` בנה את הכותרת עם "#" (מקור שם
    ההורדה של `window.print()` ב-iframe); "#" גורם לדפדפנים לקצץ את שם הקובץ →
    ריק/גנרי. הכותרת עכשיו במספר נקי ("הצעת מחיר 499 - משפחת חזן"); תווית ה-UI
    הדביקה שומרת "#". fallbacks: מספר-בלבד / לקוח-בלבד / אף אחד.
  - **היררכיית טופס** — שם-הפרק (textarea) היה בלי הדגשה בעוד שמות הפריטים
    13px/600 — היררכיה הפוכה. נוסף `.section-name-input` (15px/700); רוחב/מיקום לא נגעו.
