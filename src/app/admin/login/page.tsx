    ...
    if (data.ok) {
      ...
      setError(
        data.error === "wrong_password"
          ? "סיסמה שגויה"
          : data.error === "ADMIN_PASSWORD env var is not set"
          ? "ADMIN_PASSWORD לא מוגדר ב-Vercel"
          : "שגיאה — נסה שוב"
      );
      setPassword("");
      setTimeout(() => {
      ...
      >
        <div className="border border-warm-gray-light bg-white p-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="font-body text-xs text-charcoal/40">בנין איתן — ממשק מנהל בלבד</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? <><Loader2 size={16} className="animate-spin" /> מאמת...</> : "כניסה"}
            </button>
          </form>
