    ...
    <div className="space-y-5">
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={p.onReload} className="flex items-center gap-1 text-xs text-charcoal/40 hover:text-accent transition-colors"><RefreshCw size={12} strokeWidth={1.5} /> רענן</button>
        </div>
        {p.projects.length === 0 && <p className="text-sm text-charcoal/30 text-center py-4">אין פרויקטים</p>}
                ...
                {isEditingAddr ? (
                  ...
                  <>
                    <span className="flex-1 min-w-0 text-charcoal/60 truncate">
                      {proj.address || <span className="text-charcoal/30 italic">לא הוגדרה</span>}
                    </span>
                    {hasCoords ? (
                      <a
