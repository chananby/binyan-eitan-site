...
export default function HeError({
  error,
  reset,
}: {
    ...
    >
      <div className="max-w-lg w-full text-center">
        ...
        <p className="text-warm-gray-light mb-8 leading-relaxed">
          קרתה שגיאה לא צפויה בעת טעינת הדף. הצוות שלנו קיבל התראה ויטפל בכך. בינתיים — נסה לטעון מחדש או לחזור לדף הבית.
        </p>
        {process.env.NODE_ENV === "development" && (
          <pre
            dir="ltr"
            className="text-xs text-left bg-charcoal-light p-4 rounded mb-6 overflow-auto max-h-40 text-warm-gray-light"
          >
            {error.message}
            {error.digest && `\n\nDigest: ${error.digest}`}
          </pre>
