...
const tools = [
  ...
  },
  {
    ...
    icon: ClipboardList,
    title: "יומן יומי",
    desc: "תיעוד פעילות אתר, כוח אדם ואבני דרך",
    available: false,
  },
  {
    href: "#",
    icon: Package,
    title: "בקשת חומרים",
    desc: "הזמנת חומרים וציוד לאתר הבנייה",
    available: false,
  },
];
    ...
    <main className="min-h-screen bg-bone" dir="rtl">
      <div className="mx-auto max-w-3xl px-6 py-16 md:py-24">
              ...
              >
                <div className="flex items-start justify-between">
                  <div className={`p-2 ${tool.available ? "bg-accent/[0.08]" : "bg-charcoal/[0.05]"}`}>
                    <Icon
                      size={18}
                      className={tool.available ? "text-accent" : "text-charcoal/40"}
                      strokeWidth={1.5}
                    />
                  </div>
                  {!tool.available && (
                  ...
                  {tool.available && (
                    <ArrowRight
                      size={14}
                      className="text-charcoal/20 group-hover:text-accent transition-colors duration-200 mt-0.5"
                      strokeWidth={1.5}
                    />
                  )}
                </div>
        ...
        <div className="mt-12 pt-8 border-t border-charcoal/10">
          <Link
            href="/he"
            className="font-body text-xs text-charcoal/35 hover:text-charcoal/60 transition-colors duration-200 tracking-wide"
          >
            ← חזרה לאתר
          </Link>
        </div>
